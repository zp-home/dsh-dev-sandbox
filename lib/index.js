...
function isDesktopProfiles(value) {
	if (value === null || typeof value !== "object") return false;
	const current = value.current;
	if (current === null || typeof current !== "object") return false;
	const candidate = current;
	return typeof candidate.name === "string" && candidate.name.length > 0 && !candidate.name.includes("\0") && typeof candidate.dir === "string" && isAbsolute(candidate.dir) && !candidate.dir.includes("\0");
}
/** Narrow the documented public package-manager service shape. */
function isDesktopPnpm(value) {
...
*/
function desktopSandboxAdapter(ctx) {
	const profiles = service(ctx, "desktopProfiles");
	const pnpm = service(ctx, "desktopPnpm");
	if (!isDesktopProfiles(profiles) || !isDesktopPnpm(pnpm)) return void 0;
	return {
		hostProfile: Object.freeze({
			name: profiles.current.name,
			dir: profiles.current.dir
		}),
		buildRunner: { async build(pluginPath, signal) {
			if (!isAbsolute(pluginPath) || pluginPath.includes("\0")) throw new Error("dsh-dev-sandbox: Desktop plugin build path must be absolute and NUL-free");
			const timeout = AbortSignal.timeout(BUILD_TIMEOUT_MS);
			const buildSignal = signal === void 0 ? timeout : AbortSignal.any([signal, timeout]);
			const operation = pnpm.run([
				"--dir",
				pluginPath,
				"run",
				"build"
			...
			try {
				...
				if (outcome.signal !== null) {
					const reason = timeout.aborted ? `timed out after ${BUILD_TIMEOUT_MS}ms` : buildSignal.aborted ? "was cancelled" : "was terminated";
...
var SandboxManager = class {
	...
	buildControllers = /* @__PURE__ */ new Set();
	activeBuilds = /* @__PURE__ */ new Set();
	options;
	...
	*/
	...
	hostWebPlugins() {
		...
		if (!existsSync(manifestFile)) throw new SandboxError(`dsh-dev-sandbox: host profile is missing ${manifestFile}`);
		let manifest;
		try {
			manifest = JSON.parse(readFileSync(manifestFile, "utf8"));
		} catch {
	...
	*/
	async build(pluginPath, pnpmPath = "pnpm", signal) {
		...
		if (runner !== void 0) {
			...
			const buildSignal = signal === void 0 ? controller.signal : AbortSignal.any([signal, controller.signal]);
			let build;
			try {
	...
	*/
	create(name, pluginPath, opts = {}) {
		...
		const profile = profileMode === "host-web" ? this.mirrorHostProfile(profileDir, pluginName, pluginDir) : this.writeCleanProfile(profileDir, pluginName, pluginDir);
	...
	mirrorHostProfile(profileDir, pluginName, pluginDir) {
	...
	*/
	async start(name, port, options = {}) {
		...
		if ((options.build ?? this.options.buildOnStart) && state.pluginPath !== "") try {
			...
			if (exitCode !== 0) throw new SandboxError(`dsh-dev-sandbox: build failed with exit code ${exitCode}`);
		} catch (error) {
			...
			async execute(args) {
				...
				if (args.build === true) {
					...
					if (state !== null && state.pluginPath !== "") {
						...
						if (exitCode !== 0) throw new Error(`dsh-dev-sandbox: build failed with exit code ${exitCode}`);
					}
				}
...
*/
function mountSandboxSurfaces(ctx, config, desktop) {
	let disposeSection;
	let disposeRoutes;
	let disposeTools;
...
function hasDesktopProfiles(ctx) {
	return ctx.get("desktopProfiles") !== void 0;
}
/**
...
function apply(ctx, config) {
	if (!hasDesktopProfiles(ctx)) {
		ctx.effect(() => mountSandboxSurfaces(ctx, config), "dsh-dev-sandbox: ordinary host surfaces");
		return;
	}
	ctx.inject(["desktopProfiles", "desktopPnpm"], (desktopCtx) => {
		const adapter = desktopSandboxAdapter(desktopCtx);
		if (adapter === void 0) return;
		return mountSandboxSurfaces(desktopCtx, config, adapter);
	});
}
var src_default = {
	name,
