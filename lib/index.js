	...
	*/
	async verify(pluginPath, options = {}) {
		...
		const base = {
			...
			kind: options.repository === void 0 ? "baseline-compatibility" : "local-compatibility",
	...
	try {
		switch (action) {
			...
			case "/verify": {
				...
				json(res, 200, { verification: await manager.verify(pluginPath, {
	...
	return [
		...
		defineTool({
			...
			parameters: {
				...
				repository: {
					type: "string",
					description: "Optional marketplace repository identity in owner/repo form for the local receipt."
				}
			},
			output: {
				schema: {
			...
			async execute(args) {
				...
				return { verification: jsonRecord(await manager.verify(args.pluginPath, {
