...
function resolveHarness(configured) {
	...
	if (configured !== void 0 && configured.trim() !== "") {
		...
		if (!isCheckout(root)) throw new Error(`dsh-dev-sandbox: harnessRoot ${root} is not a dsh source checkout (apps/cli/src/bin.ts not found)`);
		return {
			root,
	...
	if (cwdRoot !== void 0) return {
		root: cwdRoot,
		cliEntry: join(cwdRoot, "apps", "cli", "src", "bin.ts"),
		nodeExec
	};
	if (import.meta.url.startsWith("file:")) {
		const ownRoot = walkUp(dirname(import.meta.url.replace(/^file:\/\//, "").split("/").map(decodeURIComponent).join("/")));
		if (ownRoot !== void 0) return {
			root: ownRoot,
			cliEntry: join(ownRoot, "apps", "cli", "src", "bin.ts"),
			nodeExec
		};
	}
	try {
		const bootRoot = walkUp(dirname(createRequire(import.meta.url).resolve("@deepseek-ai/dsh-app-boot/package.json")));
		if (bootRoot !== void 0) return {
			root: bootRoot,
			cliEntry: join(bootRoot, "apps", "cli", "src", "bin.ts"),
			nodeExec
		};
	} catch {}
	throw new Error("dsh-dev-sandbox: cannot locate the dsh installation automatically. Set \"harnessRoot\" in the dev-sandbox row config to the absolute path of the dsh source checkout (the directory that contains apps/cli/src/bin.ts).");
}
//#endregion
...
var SandboxManager = class {
	...
	*/
	logs(name, tail = 200) {
		...
		const lines = (this.rings.get(name) ?? []).join("").split("\n");
	...
	*/
	create(name, pluginPath, opts = {}) {
		...
		if (existing !== null && resolve(existing.pluginPath) === resolve(pluginDir)) return existing;
		...
		this.pushLog(name, pluginName === "" ? `[dsh-dev-sandbox] created home ${home} (plain mirror, no plugin)\n` : `[dsh-dev-sandbox] created home ${home} (plugin ${pluginName}@${pluginVersion ?? "?"})\n`);
		return state;
	}
	/** Junction `<profile>/node_modules/<pkg>` -> plugin checkout (scoped names nested). */
	ensurePluginJunction(profileDir, packageName, target) {
	...
	*/
	async start(name, port) {
		...
		if (this.options.buildOnStart && state.pluginPath !== "") try {
	...
	*/
	async stop(name) {
		...
		if (child !== void 0 && child.exitCode === null && child.pid !== void 0) {
			this.pushLog(name, "[dsh-dev-sandbox] stopping (SIGTERM)\n");
			child.kill("SIGTERM");
			if (!await Promise.race([once(child, "exit").then(() => true), delay(this.options.stopTimeoutMs).then(() => false)])) {
...
function sandboxTools(manager) {
	return [
		...
		defineTool({
			...
			async execute(args) {
				if (manager.get(args.name) === null) manager.create(args.name, args.pluginPath !== void 0 && args.pluginPath !== "" ? args.pluginPath : void 0, {
				...
				return { sandbox: await manager.start(args.name, args.port) };
			}
		}),
		defineTool({
			name: "sandbox_stop",
