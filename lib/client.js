		...
		*/
		function createPanel(api, controller, disposeRef) {
			...
			const render = () => {
				...
				if (state.error !== null) body.append(el("div", { class: "dshsb-error-box" }, state.error));
			...
			const onCreate = async () => {
				...
				render();
				try {
					if (state.sandboxes.find((item) => item.name === name) === void 0) await api.create(name, pluginPath === "" ? void 0 : pluginPath, {
			...
			const unsubscribe = controller.subscribe(() => {
				...
				if (controller.getSnapshot().panelOpen) refresh();
			});
			render();
			disposeRef.current = () => {
				unsubscribe();
